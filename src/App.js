  ...
  const TacticalPitch = ({ teamKey, canEdit }) => {
    ...
    return (
      <div className="pitch-canvas" style={{
        ...
        aspectRatio: '2/3', padding: '10px', margin: '10px auto', width: '150px'
      }}>
        {rows.map((count, r) => (
        ...
        <style dangerouslySetInnerHTML={{ __html: `.pitch-canvas { transform: rotate(0deg); } @media (min-width: 800px) { .pitch-canvas { transform: rotate(-90deg); aspect-ratio: 3/2; } .pitch-canvas div { transform: rotate(90deg); } }` }} />
      </div>
    );
  };
    ...
    <div style={{ backgroundColor: '#050505', color: '#eee', minHeight: '100vh', fontFamily: 'Arial' }}>
      {!joined ? (
        ...
        <div style={{ padding: '15px' }}>
          ...
          {/* REFEREE MASTER PANEL - PROTECTED LAYOUT */}
          {isRef && (
            <div style={{ background: '#1a1a1a', border: '1px solid gold', padding: '15px', marginTop: '10px', borderRadius: '8px' }}>
              ...
              {/* Ref Tactics View - ONLY SHOWS AFTER DRAFT */}
          ...
          {/* PHASE 3: DRAFTING - ONLY SHOWS IF NOT FINISHED */}
            ...
            <div style={{marginTop:'15px'}}>
              ...
              <div style={{display:'flex', gap:'10px'}}>
                ...
                <div style={{flex:1.5, fontSize:'0.7rem'}}>
                  <div style={{background:'#111', padding:'5px', border:'1px solid #333', marginBottom:'5px'}}><b style={{color:'#0f0'}}>T1 ({gameState.team1Picks.length}/11)</b><br/>{calcPts(gameState.team1Picks)} pts</div>
                  <div style={{background:'#111', padding:'5px', border:'1px solid #333'}}><b style={{color:'#f44'}}>T2 ({gameState.team2Picks.length}/11)</b><br/>{calcPts(gameState.team2Picks)} pts</div>
                </div>
              </div>
            </div>
          )}

          {/* PHASE 4: TACTICS - TRIGGERS ONLY WHEN 11 PICKED */}
          {gameState.gameStarted && myUser?.role?.startsWith('team') && gameState[`${myUser.role}Picks`].length === 11 && (
             <div style={{marginTop:'20px', textAlign:'center'}}>
                <h2 style={{color:'gold', margin:'0 0 10px 0'}}>STRATEGY BOARD</h2>
