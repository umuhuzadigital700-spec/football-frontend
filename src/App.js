  ...
  const TacticalPitch = ({ teamKey, canEdit }) => {
    ...
    return (
      <div className="pitch" style={{
        ...
        aspectRatio: '2/3', padding: '10px', margin: '10px auto', width: '160px', boxShadow: '0 0 10px black'
      }}>
        {rows.map((count, rIdx) => (
            ...
            {Array.from({ length: count }).map((_, i) => {
              ...
              return (
                <div key={i} onClick={() => canEdit && setActiveSlot(sIdx)}
                     style={{
                        width: '30px', height: '30px', borderRadius: '50%', border: '1px solid gold',
                        background: p ? '#111' : 'rgba(0,0,0,0.5)', color: 'white',
                        fontSize: '0.4rem', display: 'flex', alignItems: 'center',
        ...
        <style dangerouslySetInnerHTML={{ __html: `
          @media (min-width: 800px) {
            .pitch { transform: rotate(-90deg); aspect-ratio: 3/2; }
            .pitch div { transform: rotate(90deg); }
          }
        `}} />
      </div>
    );
        ...
        <div style={{ padding: '15px' }}>
          ...
          {isRef && (
            <div style={{ background: '#1a1a1a', border: '1px solid gold', padding: '10px', marginTop: '10px' }}>
              ...
              {/* Referee Monitoring Pitches - ONLY SHOWS AT THE VERY END */}
              {gameState.currentTurn === "FINISHED" && (
                <div style={{display:'flex', gap:'10px', justifyContent:'center', marginTop:'10px'}}>
                  <div style={{textAlign:'center'}}><p style={{fontSize:'0.6rem', color:'gold', margin:0}}>Team 1 Layout</p><TacticalPitch teamKey="team1" canEdit={false} /></div>
                  <div style={{textAlign:'center'}}><p style={{fontSize:'0.6rem', color:'gold', margin:0}}>Team 2 Layout</p><TacticalPitch teamKey="team2" canEdit={false} /></div>
                </div>
              )}
          ...
          {/* PHASE 3: DRAFTING - SHOWN ONLY IF NOT FINISHED */}
          ...
          {/* PHASE 4: TACTICAL BOARD - SHOWN ONLY TO PLAYERS WITH 11 CARDS */}
             ...
             <div style={{marginTop:'20px', textAlign: 'center'}}>
                ...
                {activeSlot !== null && (
                   <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.95)', zIndex: 1000, padding: '20px', overflowY:'auto' }}>
                      <h3 style={{color: 'gold'}}>Assign Player to Spot</h3>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center' }}>
                         {gameState[`${myUser.role}Picks`].map(p => (
                            <button key={p.id} onClick={() => { socket.emit('playerSetPosition', { slotIndex: activeSlot, cardId: p.id }); setActiveSlot(null); }}
                                    style={{ padding: '12px', background: '#222', color: 'white', border: '1px solid gold' }}>
                               {p.name}
                            </button>
                         ))}
                      </div>
