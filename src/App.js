  ...
  const TacticalPitch = ({ teamKey, canEdit }) => {
    ...
    return (
      <div className="pitch-ui" style={{
        ...
        {rows.map((count, r) => (
          <div key={r} style={{ display: 'flex', justifyContent: 'space-around', width: '100%' }}>
        ...
        <style dangerouslySetInnerHTML={{ __html: `.pitch-ui { transform: rotate(0deg); } @media (min-width: 800px) { .pitch-ui { transform: rotate(-90deg); aspect-ratio: 3/2; } .pitch-ui div { transform: rotate(90deg); } }` }} />
      </div>
    );
  };
      ...
      {!joined ? (
        ...
        <div style={{ padding: '15px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', background: '#111', padding: '10px', borderBottom: '2px solid gold', alignItems: 'center' }}>
            <div><div style={{fontSize: '0.7rem', color: 'gold'}}>PLAYER: {isRef ? "ERIC" : myName}</div><div style={{fontSize: '0.9rem'}}>ROLE: {myUser?.role?.toUpperCase() || "FAN"}</div></div>
          ...
          {isRef && (
            <div style={{ background: '#1a1a1a', border: '1px solid gold', padding: '10px', marginTop: '10px' }}>
              ...
              <div style={{maxHeight:'100px', overflowY:'auto', marginTop:'10px', background:'#000', padding:'5px'}}>
                {gameState.allViewers.map(v => (
                  <div key={v.id} style={{fontSize:'0.8rem', padding:'3px', borderBottom:'1px solid #222', display:'flex', justifyContent:'space-between'}}>
                    <span>{v.name}</span>
                    <div><button onClick={() => socket.emit('refAssignRole', {userId:v.id, role:'team1'})} style={{background:'lime', marginRight:'5px'}}>T1</button><button onClick={() => socket.emit('refAssignRole', {userId:v.id, role:'team2'})} style={{background:'red', color:'white'}}>T2</button></div>
                  </div>
                ))}
              </div>
              ...
              <div style={{marginTop: '10px'}}>
                <button onClick={() => socket.emit('refReset')} style={{background:'blue', color:'white', padding:'10px'}}>RESET</button>
                <button onClick={() => socket.emit('refStartDraft')} style={{background:'gold', padding:'10px', fontWeight:'bold', marginLeft:'10px'}}>START</button>
                <button onClick={() => socket.emit('refClearArena')} style={{background:'purple', color:'white', padding:'10px', marginLeft:'10px'}}>CLEAR</button>
              </div>
            </div>
          )}
            ...
            <div style={{marginTop:'15px'}}>
              ...
              <div style={{display:'flex', gap:'10px'}}>
                <div key={gameState.availableCards.length} style={{flex:2.5, display:'flex', flexWrap:'wrap', gap:'5px', maxHeight:'50vh', overflowY:'auto'}}>
                  {gameState.availableCards.map(c => <div key={c.id} onClick={() => socket.emit('playerPickCard', c.id)} style={{border:'1px solid #444', padding:'5px', width:'75px', background:'#111', fontSize:'0.7rem', cursor: (myUser?.role === gameState.currentTurn) ? 'pointer' : 'not-allowed', opacity: (myUser?.role === gameState.currentTurn) ? 1 : 0.4}}><b>{c.name}</b><br/><span style={{color:'gold'}}>{c.pos}</span><br/><span style={{color:'#0f0'}}>{c.points} pts</span></div>)}
                </div>
             ...
             <div style={{marginTop:'20px', textAlign:'center'}}>
                ...
                {activeSlot !== null && (
                   <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.95)', zIndex: 1000, padding:'20px', overflowY:'auto'}}>
                      ...
                      <button onClick={() => setActiveSlot(null)} style={{marginTop:'25px', padding:'10px 40px', background:'red', color:'white', border:'none'}}>CLOSE</button>
                   </div>
                )}
             </div>
          )}
